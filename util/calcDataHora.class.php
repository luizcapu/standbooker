...
class calcDataHora
{
	...
	{
		$this->CalcDataHora(strftime("%d/%m/%Y", mktime(0, 0, 0, $this->data[1], $this->data[0]+$dias, $this->data[2])), $this->getHora());
		return $this->data;
	}
	public function somaMes($meses=1)
	{
		$this->CalcDataHora(strftime("%d/%m/%Y", mktime(0, 0, 0, $this->data[1]+$meses, $this->data[0], $this->data[2])), $this->getHora());
		return $this->data;
	}
	public function somaAno($anos=1)
	{
		$this->CalcDataHora(strftime("%d/%m/%Y", mktime (0, 0, 0, $this->data[1], $this->data[0], $this->data[2]+$anos)), $this->getHora());
		return $this->data;
	}
	...
	public function somaHoraComData($horas=1)
	{
		$this->CalcDataHora($this->getData(),strftime("%H:%M:%S", mktime($this->hora[0]+$horas, $this->hora[1], $this->hora[2], 0, 0, 0)));
		return $this->hora;
	}
	...
	public function difDataHora($datamenor,$datamaior="",$tipo="")
	{
		...
		if ($segundomenor=="") $segundomenor = 0;
		if ($segundomaior=="") $segundomaior = 0;
